...
def parse_opt(known=False):
    ...
    # 가중치 모델 선정 (yolo5 - n, m, l, x)
    parser.add_argument('--weights', type=str, default='yolov5l.pt', help='initial weights path')
    # cfg 파일 위치(기본값 사용)
    parser.add_argument('--cfg', type=str, default='', help='model.yaml path')
    # 학습/평가 데이터 경로 및 클래스 정의
    parser.add_argument('--data', type=str, default='data/type_2.yaml', help='dataset.yaml path')
    # 하이퍼파라미터 정의(기본값 사용)
    parser.add_argument('--hyp', type=str, default='data/hyps/hyp.scratch-low.yaml', help='hyperparameters path')
    # 학습 반복 횟수 정의 
    parser.add_argument('--epochs', type=int, default=100)
    # 배치 사이즈 정의 -1은 자동으로 메모리에 맞게 설정됨
    parser.add_argument('--batch-size', type=int, default=-1, help='total batch size for all GPUs, -1 for autobatch')
    # 입력 이미지 해상도 크기
    parser.add_argument('--imgsz', '--img', '--img-size', type=int, default=3000, help='train, val image size (pixels)')
    # 이하 수정 불필요
